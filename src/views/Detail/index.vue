...
const addCart = () =>{
  if(skuObj.skuId) {
    ...
    ElMessage.warning('请先选择规格')// 一直点击会重复出现警告，记得防抖
    // 防抖
  }
      ...
      <div class="bread-container">
        ...
        <el-breadcrumb separator=">">
          ...
          <!-- goods一开始是一个空对象
           ...
           <!-- <el-breadcrumb-item :to="{ path: `/category/${goodsData.categories?.[1].id}` }">{{goodsData.categories?.[1].name}} -->
          <el-breadcrumb-item :to="{ path: `/category/${goodsData.categories[1].id}` }">{{goodsData.categories[1].name}}
          </el-breadcrumb-item>
          <el-breadcrumb-item :to="{ path: `/category/sub/${goodsData.categories[0].id}` }">{{goodsData.categories[0].name}}
          </el-breadcrumb-item>
      ...
      <div class="info-container">
        <div>
          <div class="goods-info">
            <div class="media">
              <!-- 图片预览区 -->
              <img-view :image-list="goodsData.mainPictures"></img-view>
              <!-- 统计数量 -->
            ...
            <div class="spec">
              ...
              <!-- sku组件 -->
              <xtx-sku :goods="goodsData" @change="skuChange"></xtx-sku>
              <!-- 数据组件 -->
              <el-input-number v-model="count" :min="1" :max="10" @change="countChange" />
              <!-- 按钮组件 -->
              <div>
                <el-button size="large" class="btn" @click="addCart" type="primary" :icon="ShoppingCart">
                  加入购物车
                </el-button>
              </div>
          ...
          <div class="goods-footer">
            <div class="goods-article">
              ...
              <div class="goods-tabs">
                ...
                <div class="goods-detail">
                  ...
                  <!-- 图片 -->
                  <img v-for="img in goodsData.details.pictures" :src="img" :key="img" alt="">
                </div>
              </div>
            </div>
            <!-- 24热榜+专题推荐 -->
            <div class="goods-aside">
              <!-- 24小时 -->
              <detail-hot :hot-type="1" title="24小时热榜"></detail-hot>
              <!-- 周 -->
              <detail-hot :hot-type="2" title="周热榜"></detail-hot>
            </div>
          </div>
        </div>
      </div>
